    ...
    switch (action) {
      case 'config':
        ...
        if (!geminiKey || geminiKey === 'your_gemini_api_key_here') {
          return NextResponse.json({
            success: false,
            error: 'Gemini API key not configured. Please set NEXT_PUBLIC_GEMINI_API_KEY in environment variables.'
          });
        }
      ...
      case 'test':
        return NextResponse.json({
          ...
          allConfigured: !!(process.env.DEEPGRAM_API_KEY && process.env.GEMINI_API_KEY),
      ...
      case 'models':
        return NextResponse.json({
          ...
          ttsModels: ['aura-asteria-en', 'aura-luna-en', 'aura-stella-en', 'aura-kara-en'],
      ...
      default:
        return NextResponse.json(
          { error: 'Invalid action', availableActions: ['config', 'test', 'models'] },
          { status: 400 }
        );
    }
  } catch (error) {
...
async function validateConfiguration(config) {
  ...
  return NextResponse.json({
    ...
    message: errors.length === 0 ? 'Configuration is valid' : 'Configuration errors found'
  });
}
...
async function testConnections() {
  ...
  try {
    ...
    return NextResponse.json({
      success: true,
      results,
      message: results.overall ? 'All connections successful' : 'Some connections failed'
    });
  } catch (error) {
    return NextResponse.json({
      success: false,

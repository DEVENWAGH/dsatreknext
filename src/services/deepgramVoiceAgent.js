...
class DeepgramVoiceAgent {
  ...
  async startListening() {
    ...
    try {
      ...
      this.deepgramSocket.onopen = () => {
        console.log('🎤 Deepgram WebSocket connected - listening for speech...');
        this.updateStatus('listening');
        ...
        if (data.channel?.alternatives?.[0]?.transcript) {
          ...
          if (!data.is_final && transcript.trim()) {
            ...
            this.interimTimeout = setTimeout(() => {
              if (transcript.trim() && transcript.length > 1 && !this.isProcessing) {
                console.log('⏰ Processing interim as final (timeout):', transcript);
  ...
  async generateResponse(userInput) {
    try {
      ...
      const questionsContext = availableQuestions.map((q, i) => {
      ...
      if (nextQuestion && this.conversation.length <= 2) {
        ...
        const questionText = nextQuestion.question || nextQuestion.text || nextQuestion;
...
All main questions have been covered. Ask a relevant follow-up question about ${this.interviewConfig.position}. Use ${this.interviewConfig.candidateName}'s name. Do not use markdown formatting or show internal analysis. Speak naturally as an interviewer.`;
  ...
  addToConversation(role, content) {
    ...
    console.log('💬 Added to conversation:', { role, content: content.substring(0, 50) + '...', total: this.conversation.length });
  ...
  async endInterview() {
    ...
    return {
      ...
      candidateResponses: this.conversation.filter(m => m.role === 'user').length,
      interviewerQuestions: this.conversation.filter(m => m.role === 'assistant').length
    };
  }
